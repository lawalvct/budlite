...
<div class="bg-gray-50 min-h-screen">
    ...
    <div class="bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 shadow-xl">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-white mb-2">Edit Position</h1>
                    <p class="text-blue-100 text-lg">Update position details</p>
                </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
            ...
            <form action="{{ route('tenant.payroll.positions.update', [$tenant, $position]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="code" value="{{ old('code', $position->code) }}" required maxlength="50"
                                   class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="level" required
                                ...
                                <option value="1" {{ old('level', $position->level) == '1' ? 'selected' : '' }}>1 - Entry Level</option>
                                <option value="2" {{ old('level', $position->level) == '2' ? 'selected' : '' }}>2 - Junior</option>
                                <option value="3" {{ old('level', $position->level) == '3' ? 'selected' : '' }}>3 - Mid-Level</option>
                                <option value="4" {{ old('level', $position->level) == '4' ? 'selected' : '' }}>4 - Senior</option>
                                <option value="5" {{ old('level', $position->level) == '5' ? 'selected' : '' }}>5 - Lead</option>
                                <option value="6" {{ old('level', $position->level) == '6' ? 'selected' : '' }}>6 - Manager</option>
                                <option value="7" {{ old('level', $position->level) == '7' ? 'selected' : '' }}>7 - Senior Manager</option>
                                <option value="8" {{ old('level', $position->level) == '8' ? 'selected' : '' }}>8 - Director</option>
                                <option value="9" {{ old('level', $position->level) == '9' ? 'selected' : '' }}>9 - Senior Director</option>
                                <option value="10" {{ old('level', $position->level) == '10' ? 'selected' : '' }}>10 - Executive</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="reports_to_position_id"
                                ...
                                @foreach($parentPositions as $parentPosition)
                                    <option value="{{ $parentPosition->id }}" {{ old('reports_to_position_id', $position->reports_to_position_id) == $parentPosition->id ? 'selected' : '' }}>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="min_salary" value="{{ old('min_salary', $position->min_salary) }}" step="0.01" min="0"
                                   class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                   placeholder="0.00">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="max_salary" value="{{ old('max_salary', $position->max_salary) }}" step="0.01" min="0"
                                   class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                   placeholder="0.00">
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea name="requirements" rows="4"
                                      class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                      placeholder="List the key requirements for this position">{{ old('requirements', $position->requirements) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="responsibilities" rows="4"
                                      class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                      placeholder="List the key responsibilities for this position">{{ old('responsibilities', $position->responsibilities) }}</textarea>
                        </div>
                    </div>
                </div>
