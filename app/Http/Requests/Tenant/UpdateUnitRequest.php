...
class UpdateUnitRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'base_unit_id' => [
                ...
                'exists:units,id',
                'different:' . $unitId,
                function ($attribute, $value, $fail) use ($tenantId, $unitId) {
                    if ($value && !\App\Models\Unit::where('id', $value)->where('tenant_id', $tenantId)->where('is_base_unit', true)->exists()) {                        $fail('The selected base unit is invalid.');
                    }

                    // Prevent circular reference
                    if ($value && $this->wouldCreateCircularReference($unitId, $value)) {
    ...
    public function messages(): array
    {
        return [
            ...
            'base_unit_id.different' => 'A unit cannot be its own base unit.',
    ...
    /**
     * Check if the proposed base unit would create a circular reference.
     */
    private function wouldCreateCircularReference($unitId, $baseUnitId): bool
    {
        $visited = [];
        $current = $baseUnitId;
        ...
        while ($current && !in_array($current, $visited)) {
            ...
            $visited[] = $current;
            $unit = \App\Models\Unit::find($current);
            $current = $unit ? $unit->base_unit_id : null;
        }
